{ pkgs, username, ... }:
{
  ...
  homebrew = {
    ...
    casks = [
      ...
      # Hammerspoon は compass（自作のアプリランチャー）へ置き換えたため外した。
  ...
  # ----------------------------------------
  # comet と compass は各マシンのローカル設定で launchd agent として登録している
  # （どちらもアクセシビリティ権限のためアプリ本体を Nix store に置かない。
  # 本体は各リポジトリの scripts/install-app.sh が ~/Applications へ入れる）
